...
struct Vector
{
	...
	void Rotate(Vector* v, double phi) //add phi
	{
		...
		x = v->dx;
		y = v->dy;
		v->dx = (SHORT)(x * cos(phi) - y * sin(phi));
		v->dy = (SHORT)(x * sin(phi) + y * cos(phi));
	}
} v[3];

struct Triangle
{
	POINTS Origin; // левый нижний угол
	Triangle* Parent;
	int level;
	Triangle* Child[3];
};
...
{
	HBRUSH hBrush; //объявление кисти
	hBrush = CreateSolidBrush(RGB(0, 0, 0)); //сплошная кисть, но есть и другие. обмакнули в краску
	SelectObject(hdc, hBrush); //взяли в руку
	Rectangle(hdc, 0, 0, 800, 800); //нарисовали прямоугольник белого цвета
	DeleteObject(hBrush); //удалили кисточку
}
...
void DrawTriangle(HDC hdc, Triangle* ptr)
{
	...
	for (int i = 0; i < 3; i++)
	{
		LineA(hdc, &(ptr->Origin), v[i].dx / koef, v[i].dy / koef);
	}
	DeleteObject(hPen);
}
...
void DivideTriangle(Triangle* ptr) 
{
	...
	ChildPtr[1]->Origin.x = ptr->Origin.x + v[0].dx / koef;
	ChildPtr[1]->Origin.y = ptr->Origin.y - v[0].dy / koef;

	ChildPtr[2]->Origin.x = ptr->Origin.x - v[2].dx / koef;
	ChildPtr[2]->Origin.y = ptr->Origin.y + v[2].dy / koef;
}
...
{ 
	...
	SelectObject(hdc, hPen);
	LineDv(hdc, &(ptr->Child[1])->Origin, v[1].dx / koef, v[1].dy / koef);
	LineDv(hdc, &(ptr->Child[1])->Origin, -v[2].dx / koef, -v[2].dy / koef);
	LineDv(hdc, &(ptr->Child[2])->Origin, v[0].dx / koef, v[0].dy / koef);
	DeleteObject(hPen);
}
...
Triangle* InitTriangle(HWND hWnd, POINTS* ptsBegin, POINTS* ptsEnd)
{
	...
	dy = - (ptsEnd->y - ptsBegin->y); // тк начало отсчета в левом верхнем углу
...
{
	if (ptr->level >= n) return;
	else if (ptr->Child[0] != NULL) //есть дети?
	{
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONUP: 
	{
		...
		fPrevLine = FALSE;
		ClipCursor(NULL); //мышка может уходить за рамки
		ReleaseCapture();
	...
	case WM_COMMAND:
	{
		...
		switch (wmId)
		{
		case IDM_ABOUT: //жму о программе -> вылетают диалогбоксы
	...
	case WM_KEYDOWN:
		...
		case VK_DOWN:
			//n--; меньше треугольников, если n<=0 => 
			if (n > 0)
			{
				FreeLevel(hdc,root);
				n--;
